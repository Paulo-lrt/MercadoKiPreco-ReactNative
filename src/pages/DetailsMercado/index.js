...
export default function DetailsMercado({ navigation, route }) {

    const { imageSource } = route.params;

    const imageArrayMercado = Object.values(imagensMercado);

    const handleImagePressMercado = (imageSource) => {
        navigation.navigate('DetailsMercado', { imageSource });
    };
            ...
            <View>
                ...
                <ScrollView horizontal={true} showsHorizontalScrollIndicator={false}>
                    {imageArrayMercado.map((imageSource, index) => (
                        <TouchableOpacity key={index} onPress={() => handleImagePressMercado(imageSource)}>
                            <Image
                                source={imageSource}
